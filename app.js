...
const muteAudio = () => {
  // Mutes or unmutes all the audio (music and end game music)
...
const setStartingPlayer = (firstPlayer) => {
  if (play_board.every((item) => item === "")) {
    if (firstPlayer === "X" || firstPlayer === "😀") {
      if (gameMode == 1) {
        showPlayer(1, 1); // Show player 1's move in single player mode
      } else {
        showPlayer(2, 1); // Show player 1's move in two player mode
      }
      console.log("PLAYER STARTS");
      ...
      if (gameMode == 1) {
        ...
        showPlayer(2, 2); // Show player 2's move in two player mode
      }
